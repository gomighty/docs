...
```astro title="resources/astro/layouts/Layout.astro"
---
import { shared } from "@mighty/laravel";
...
<html>
  ...
  <body>
    <header>
      {user !== null ? `Welcome back, ${name}!` : "Please log in!"}
    </header>
    <main><slot /></main>
  </body>
</html>
...
### Using the Astro context

You can use Astro context properties, such as `Astro.request` and `Astro.params`, just as you would in a standard Astro project.
...
- Cannot use Astro.response to set status code, e.g. `Astro.response.status = 404` won't work
...
- user('guard') for getting the current user
- session('key') and errors('errorBag') to get session info
  - Proper session support is planned
- csrfToken() and `<CSRF />` for form submission
  - Action support with CSRF is planned
- `<Method put />` for form method spoofing
