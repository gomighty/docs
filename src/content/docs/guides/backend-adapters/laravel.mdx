...
<Aside>This part is a work in progress, as we add more and more utilities!</Aside>
...
- A `user` function for getting the current user:
  ...
  ```astro
  ---
  import { user } from "@mighty/laravel";

  const currentUser = user<{ id: number; name: string } | null>(); // or user('guard')
  ---

  {currentUser !== null ? `Welcome back, ${currentUser.name}!` : "Please log in!"}
  ```
...
- A `session` function to get session info:
  ...
  ```astro
  ---
  import { session } from "@mighty/laravel";
...
- An `errors` function to get validation errors:
  ...
  ```astro
  ---
  import { errors } from "@mighty/laravel";
...
- A `csrfToken` function and a `<CSRF>` component for form submission:
  ...
  ```astro
  ---
  import { csrfToken, CSRF } from "@mighty/laravel";
  ---
...
- A `<Method>` component for form method spoofing:
  ...
  ```astro
  ---
  import { CSRF, Method } from "@mighty/laravel";
  ---
